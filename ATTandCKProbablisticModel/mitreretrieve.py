#!/usr/bin/env python3
"""
Module Name: mitreretrieve.py
Description: Creates a spreadsheet of techniques for each threat actor listed in mitre
...
templatearraymitretypes=["reconnaissance", "resource-development", "initial-access", "execution", "persistence", "privilege-escalation", "defense-evasion", "credential-access", "discovery", "lateral-movement", "collection","command-and-control","exfiltration","impact"]
ta_to_technique_dictionary = {}
...
def retrieve_techniques(data):
    techniques_to_tactics_array = [[],[],[],[],[],[],[],[],[],[],[],[],[],[]]
    for i in data:
        index = templatearraymitretypes.index(i["object"]["kill_chain_phases"][0]["phase_name"])
        techniques_to_tactics_array[index].append(i["object"]["external_references"][0]["external_id"])
    return techniques_to_tactics_array

# Example usage:
  # G0019
def retrieve_id_to_name_mapping(thesrc):
    return thesrc.query([
        Filter('type', '=', 'intrusion-set')
    ])

def create_id_to_name_dict(thesrc):
    id_to_name_dict = {}
    x = retrieve_id_to_name_mapping(thesrc)
    for y in x:
        id_to_name_dict[y["id"]] = y["name"]
    return id_to_name_dict



def add_actor_to_technique_dictionary(taid, techniquestotactics):
    ta_to_technique_dictionary[taid] = techniquestotactics

# https://www.geeksforgeeks.org/python-create-and-write-on-excel-file-using-xlsxwriter-module/
def generate_xlsxfile(id_to_name_dict):
    workbook = xlsxwriter.Workbook('threatactortechniques.xlsx')
    worksheet = workbook.add_worksheet()
    worksheet.write(0,0,"Threat Actor ID")
    worksheet.write(0,1,"Threat Actor Name")
    worksheet.write(0,2,"reconnaissance")
    worksheet.write(0,3,"resource-development")
    worksheet.write(0,4,"initial-access")
    worksheet.write(0,5,"execution")
    worksheet.write(0,6,"persistence")
    worksheet.write(0,7,"privilege-escalation")
    worksheet.write(0,8,"defense-evasion")
    worksheet.write(0,9,"credential-access")
    worksheet.write(0,10,"discovery")
    worksheet.write(0,11,"lateral-movement")
    worksheet.write(0,12,"collection")
    worksheet.write(0,13,"command-and-control")
    worksheet.write(0,14,"exfiltration")
    worksheet.write(0,15,"impact")

    row = 1
    for key, value in ta_to_technique_dictionary.items():
        worksheet.write(row,0,key)
        worksheet.write(row, 1, id_to_name_dict[key])
        for column in range(2,16):
            input_value = ", ".join(value[column-2])
            worksheet.write(row, column, input_value)
        row+=1
    workbook.close()


src = FileSystemSource('./cti-master/enterprise-attack')
groups_to_techniques = techniques_used_by_groups(src)
id_to_name_dict=create_id_to_name_dict(src)
for threat_actor_id in id_to_name_dict.keys():
    try:
        techniques_to_tactics = retrieve_techniques(groups_to_techniques[threat_actor_id])
    except KeyError:
        print(threat_actor_id + "has no techniques listed")
    add_actor_to_technique_dictionary(threat_actor_id, techniques_to_tactics)
print(ta_to_technique_dictionary)
generate_xlsxfile(id_to_name_dict)

# print(retrieve_techniques(groups_to_techniques["intrusion-set--3ea7add5-5b8f-45d8-b1f1-905d2729d62a"]))
